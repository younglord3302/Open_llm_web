...
export default function App() {
  ...
  useEffect(() => {
    ...
    fetchModels();
    fetchDocuments();
  }, [user, fetchChats, fetchModels, fetchDocuments]);
    ...
    <div className="app-container">
      ...
      <aside className="sidebar">
        <div className="sidebar-header">
          ...
          <span className="logo-text">OpenLLM UI</span>
        </div>
        ...
        <div className="chat-history">
          ...
          {chats.map(item => (
            <div
              key={item.id}
              className={`chat-history-item ${activeChat?.id === item.id && view === 'chat' ? 'active' : ''}`}
              onClick={() => { setActiveChat(item); setView('chat'); }}
            >
              <div className="chat-title-text">{item.title}</div>
              <button className="delete-chat-btn" onClick={(e) => handleDeleteChat(item.id, e)}>
                <Trash2 size={13} />
              </button>
            </div>
          ))}
        ...
        <div className="sidebar-footer">
          {[
            ...
            <div
              ...
              onClick={() => {
                if (nav.id === 'chat' && !activeChat && chats.length > 0) setActiveChat(chats[0]);
                setView(nav.id);
              }}
            >
              {nav.icon}
      ...
      <main className="main-content">
        <header className="top-nav">
          <div className="nav-section">
            <span style={{ fontWeight: 600, color: 'var(--text-muted)' }}>Workspace</span>
            <span style={{ color: 'var(--border-subtle)' }}>/</span>
            <span style={{ textTransform: 'capitalize', fontWeight: 600 }}>{view}</span>
          </div>

          <div className="nav-section" style={{ gap: '12px' }}>
            <div className="model-pill" onClick={() => setView('models')}>
              <span className={`status-dot ${activeModelDetails.provider === 'mock' ? 'offline' : ''}`} />
            ...
            <div style={{ position: 'relative' }}>
              ...
              {userMenuOpen && (
                <div className="user-dropdown">
                  ...
                  <button className="user-dropdown-item" onClick={() => { setView('settings'); setUserMenuOpen(false); }}>
        ...
        {view === 'dashboard' && (
          <Dashboard stats={getDashboardStats()} setView={setView} createNewChat={createNewChat} />
        )}
        {view === 'chat' && (
          <ChatWorkspace
            chat={activeChat}
            ...
            onSendMessage={(msg) => {
              if (msg === null) {
                ...
                setMessages(prev => [
                  ...prev,
                  { id: Math.random().toString(36), role: 'user', content: msg, timestamp: new Date().toISOString() }
                ]);
              }
            }}
        ...
        {view === 'agents' && (
          <AgentMarketplace createNewChat={createNewChat} onAuthError={handleAuthError} />
        )}
        {view === 'webdev' && (
          <WebDevWorkspace
            models={models}
